...
const Papers = () => {
  ...
  const handleSendForReview = async (
    ...
    try {
      // Access the selected reviewers for the specific row
  ...
  const isOptionSelected = (value: string): boolean => {
    return assignedReviewers.some((reviewers) => reviewers.includes(value));
  };
                    ...
                    <DropdownMenuContent className="w-full">
                      {users.map((user, index) => (
                        <DropdownMenuCheckboxItem
                          ...
                          onCheckedChange={() =>
                            handleSelectChange(user.id, idx, index)
                          }
                          className="w-56"
                        >
