...
const Papers = () => {
  ...
  const handleSendForReview = async (
    ...
    try {
      const dataToSend = {
        ...paperData,
        assignedReviewers: selectedItems,
      };
  ...
  const handleSelectChange = (
    value: string,
    paper: PaperSubmissionDataTypeWithIds,
    dataIndex: number,
    index: number
  ) => {
    if (!selectedItems.includes(value)) {
      ...
      setAssignedReviewers((prevData) => {
        ...
        const set = new Set(updatedAssignedReviewers[dataIndex]); // Convert to Set
        set.add(value); // Add the new value
        updatedAssignedReviewers[dataIndex] = Array.from(set); // Convert back to array
        return updatedAssignedReviewers;
      });
    } else {
              ...
              <TableRow key={idx}>
                    ...
                    <DropdownMenuContent className="w-full">
                      {users.map((user, index) => (
                        <DropdownMenuCheckboxItem
                          ...
                          onCheckedChange={() =>
                            handleSelectChange(user.id, row, idx, index)
                          }
                          className="w-56"
                        >
                ...
                <TableCell>
                  <Button
                    ...
                    onClick={() => handleSendForReview(row, selectedItems)}
                  >
                    Send for review
                  </Button>
                </TableCell>
