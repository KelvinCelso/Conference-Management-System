...
const MyConference = () => {
  const [paperDialogState, setPaperDialogState] = useRecoilState(PaperDialog);
  ...
  const [foundPaper, setFoundPaper] =
    useState<PaperSubmissionDataTypeWithIdsAndPaperId>();
  ...
  const handleDialogOpen = (projectId: string) => {
    ...
    const userPaper = userData.submittedPapers?.find(
      (p) => p.projectId == projectId
    );

    setPaperDialogState(true);
    setFoundPaper(userPaper);
  };
            ...
            {userAppliedProjectsData.map((project, idx) => {
              ...
              return (
                <TableRow key={idx}>
                  ...
                  <TableCell className="w-72">
                    <Button
                      className={`${!pr ? "bg-green-500" : "bg-green-800"}`}
                      onClick={() => handleDialogOpen(project.projectId)} // Set the selected project ID and open the dialog
                    >
