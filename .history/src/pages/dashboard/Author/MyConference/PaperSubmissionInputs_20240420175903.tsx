...
const PaperSubmissionInputs: React.FC<PaperSubmissionInputsProps> = ({
  ...
  const { submitPaper, isSubmitting, error, updatePaper } =
    usePaperSubmission();
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      abstract: paper ? paper.abstract : "", // Prefill abstract if paper exists
      coAuthor: paper ? paper.authors : [], // Prefill authors if paper exists
      correspondingAuthor: paper ? paper.correspondingAuthor : "", // Prefill corresponding author if paper exists
    ...
    <>
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-2 ">
          ...
          <Button
            className={`w-full ${!paper ? "bg-green-500" : "bg-green-800"}`}
            type="submit"
